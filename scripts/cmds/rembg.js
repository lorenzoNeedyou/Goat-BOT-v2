...
module.exports = {
    ...
    onStart: async function ({ api, args, message, event }) {
        ...
        if (bypassmain.includes(bypassmUid)) {
            console.log(`User ${bypassmUid} is in the bypass list. Skipping the main approval check.`);
        } else {
            let imageUrl;
            let type;
            ...
            try {
                ...
                const errorMessage = "----RemoveBG Log----\Something is causing an error with the removebg command.\Please check if the API key has expired.\Check the API key here: https://www.remove.bg/dashboard";
